...
export function SiteHeader({ user, sessionLoading, onLogout }: { user: SessionUser | null; sessionLoading: boolean; onLogout: () => Promise<void> }) {
  return <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
    <a href="#page-content" className="absolute left-3 top-2 z-50 -translate-y-20 rounded-md bg-slate-950 px-3 py-2 text-sm font-semibold text-white focus:translate-y-0 focus:outline-none focus:ring-2 focus:ring-teal-400">
      Skip to content
    </a>
