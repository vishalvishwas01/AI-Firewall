import { BrainCircuit, Flag, Headphones, LogIn, Server, ShieldCheck } from "lucide-react"
...
export function AdminPage() {
  ...
  return <section id="page-content" tabIndex={-1} className="min-h-[calc(100vh-4rem)] bg-[#faf9f6] outline-none"><div className={`grid min-h-[calc(100vh-4rem)] ${sidebarOpen ? "md:grid-cols-[260px_minmax(0,1fr)]" : "md:grid-cols-[76px_minmax(0,1fr)]"}`}><aside className="border-b border-[#d6d0c6] bg-[#f4f1eb] p-3 md:border-b-0 md:border-r"><div className="flex items-center justify-between"><div className={sidebarOpen ? "px-2" : "hidden"}><p className="font-[Manrope] text-sm font-semibold text-[#33312b]">Admin console</p><p className="mt-0.5 text-xs text-[#817c73]">Private control plane</p></div><button type="button" onClick={() => dispatch(toggleAdminSidebar())} aria-label={sidebarOpen ? "Close admin menu" : "Open admin menu"} className="flex h-11 w-11 items-center justify-center rounded-xl border border-[#d6d0c6] bg-white text-[#33312b]">☰</button></div><nav className="mt-5 grid gap-2" aria-label="Administration">{nav("features", "Feature availability", <Flag className="h-5 w-5" />)}{nav("help-desk", "Help desk", <Headphones className="h-5 w-5" />, unread)}{nav("verification", "Verification", <ShieldCheck className="h-5 w-5" />)}{nav("login-activity", "Login activity", <LogIn className="h-5 w-5" />)}{nav("server-logs", "Server logs", <Server className="h-5 w-5" />)}{nav("ml-workflow", "ML workflow", <BrainCircuit className="h-5 w-5" />)}</nav></aside><div className="min-w-0">{section === "features" ? <FeatureAvailabilityPanel /> : null}{section === "help-desk" ? <HelpDeskPanel active /> : null}{section === "verification" ? <VerificationCampaignPanel /> : null}{section === "login-activity" ? <LoginActivityPanel /> : null}{section === "server-logs" ? <ServerLogsPanel /> : null}{section === "ml-workflow" ? <MlWorkflowPanel /> : null}</div></div></section>
}
