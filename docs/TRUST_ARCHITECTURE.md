...
| Extension health when authenticated | Extension, policy, and intelligence versions; `active` or `protection-unavailable`; server timestamp. No browsing/content fields. |
...
As of E16, organization-managed sites may carry an exact, versioned policy for category, minimum severity, action, destination class, override permission, and redaction permission. Owner/admin writes are authorized server-side; stale/non-increasing versions conflict. The client validates and bridges the policy, and the extension validates it again before local storage. Precedence is incomplete-scan hard limit, then matching organization policy, then personal settings and local risk. Managed categories cannot be disabled personally, non-overridable blocks omit the bypass action, and intelligence packages have no policy field or override path. Legacy site-only records remain readable but do not gain fabricated enforcement policy.

As of E17, an authenticated extension sends a bounded heartbeat at startup and at most every 12 hours with version/status metadata only. The server retains the latest record for up to 90 days and reports `active`, `stale`, or `protection-unavailable`; after 36 hours without a heartbeat it reports stale, never uninstalled. Silence can mean a closed/offline browser, a disabled or removed extension, or browser failure. HallGuard itself cannot prevent removal; enterprise anti-bypass requires managed-browser/device force-install controls outside the product.

As of E18/S13, the automated local lifecycle drill covers ephemeral offline signing, reviewed publication gates, authenticated retrieval, Ed25519 verification, SHA-256 binding, trusted/revoked/expired keys, schema/data-only restrictions, compatibility, monotonic sequence, atomic activation, last-known-good recovery, explicit higher-sequence rollback, replay rejection, and bundled fallback. Active package/model/rule/trust versions are metadata-visible while inference remains local. Signed packages cannot contain policy, consent, redaction, executable, or threshold overrides. Production rollout remains blocked until the deployment-owned staging drill is completed with externally custodied keys and reviewed publisher identities.
...
B1 adds a content-free provenance and review layer before any external benign corpus can enter the ML workspace. Candidate metadata covers CPython, the Kubernetes website, and Node.js repositories across source-code, configuration, and documentation content. Every candidate is `not-downloaded`, has null immutable revision/archive digest fields, and requires independent privacy, security, and maintainer review. Path allowlists, exclusions, quarantine scanning, grouped splitting, retention/deletion, license verification, and attribution are mandatory evidence items. B1 does not change the M3 report or clear a release gate; approved pins and real reviewer evidence are prerequisites for separately authorized B2 intake.
