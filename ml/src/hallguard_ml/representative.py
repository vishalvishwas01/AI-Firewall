...
from .contracts import FINAL_REMEDIATION_APPROVAL_VERSION, validate_final_remediation_approval, validate_representative_set_evidence
...
PATTERNS = {
    "hashes-uuids-and-timestamps": re.compile(r"(?i)(?<![0-9a-f])[0-9a-f]{32,64}(?![0-9a-f])|\b[0-9a-f]{8}-[0-9a-f-]{27,}\b|\b\d{4}-\d{2}-\d{2}(?:[T ][0-9:.+\-Z]+)?\b"),
    "paths-urls-and-versions": re.compile(r"https?://[^\s\"'<>]{4,160}|\b[vV]?\d+(?:\.\d+){1,3}(?:[-+][a-z0-9.-]+)?\b|(?:[A-Za-z]:)?[/\\][A-Za-z0-9_.\\/-]{3,120}"),
...
def _excluded(relative: Path, text: str, profile: dict[str, Any]) -> bool:
    return bool(_secondary_reasons(text, profile)) or any(pattern.search(text) for pattern in NOTICE_CATEGORIES.values())
    ...
    for path in sorted(item for item in root.rglob("*") if item.is_file()):
        ...
        for stratum, pattern in PATTERNS.items():
            for match in pattern.finditer(text):
                ...
                if stratum == "ordinary-identifiers" and (value.lower() in PLACEHOLDER_WORDS or KEYWORD_PATTERN.search(context)):
                    continue
                candidates.append((family, stratum, value, context))
                if stratum == "ordinary-identifiers" and KEYWORD_PATTERN.search(context) and value.lower() in PLACEHOLDER_WORDS:
                    candidates[-1] = (family, "secret-keyword-context-with-benign-values", value, context)
