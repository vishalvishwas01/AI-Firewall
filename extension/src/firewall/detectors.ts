import type { Detection, FileSummary, ProtectionSettings } from "./types"
...
export const detectSensitiveData = (text: string): Detection[] => {
  ...
  if (assignmentSecretPattern.test(text)) {
    evidence.push("secret assignment")
    severity = "high"
  }
  if (sensitiveUrlAssignmentPattern.test(text)) {
    evidence.push("sensitive service URL assignment")
    severity = "high"
  }
  if (genericTokenPattern.test(text)) {
