...
const assignmentSecretPattern =
  /\b(api[_-]?key|access[_-]?token|secret|password|passwd|pwd|token)\b\s*[:=]\s*["']?([^\s"',;]{6,})/gi
const sensitiveUrlAssignmentPattern =
  /\b((?:[a-z0-9]+[_-])?(?:database|db|supabase|firebase|webhook|callback|redirect|site|service|api)[_-]?url)\b\s*[:=]\s*["']?([^\s"',;]{4,})/gi
