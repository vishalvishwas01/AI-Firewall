...
import {
  activateOrganizationInvitations,
...
export const registerEnterpriseUser = async (
  db: Db,
  email: string,
  password: string,
  name: string,
  ...
  const user = await createUser(db, email, await bcrypt.hash(password, 12), "enterprise", name);
  const now = new Date();
  const organization = await organizationsCollection(db).insertOne({
    name: companyName,
    ownerUserId: userId(user),
    createdAt: now,
    updatedAt: now,
  });

  await organizationMembersCollection(db).insertOne({
...
export const authenticateGoogleUser = async (
  ...
  } else if ((user.accountType ?? "individual") !== accountType) {
    return undefined;
  }
