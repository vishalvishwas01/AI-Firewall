...
export const findUserByGoogleId = (db: Db, googleId: string) =>
  usersCollection(db).findOne({ googleId });
...
export const linkGoogleAccount = async (
  ...
  const updatedUser = await usersCollection(db).findOne({
    _id: user,
  });

  if (!updatedUser) {
    throw new Error("User could not be loaded after linking Google account");
  }
